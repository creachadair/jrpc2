...
func resultDecoder(rspType reflect.Type) func(*jrpc2.Response, error) []reflect.Value {
	// There is no result type; just return the error value, or nil.
	if rspType == nil {
		return func(_ *jrpc2.Response, err error) []reflect.Value {
	...
	if wantPtr {
		...
		return func(rsp *jrpc2.Response, err error) []reflect.Value {
